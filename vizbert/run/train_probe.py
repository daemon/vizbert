...
def main():
    def evaluate(loader):
        ...
        with torch.no_grad():
            for batch in pbar:
                scores_batch = probe(batch.hidden_states.to(args.device)).cpu()
    ...
    parser.add_argument('--layer-idx', '-l', type=int, default=5)
    ...
    for _ in trange(args.num_epochs, position=0):
        ...
        dev_uuas = evaluate(dev_loader)
        if dev_uuas <= last_dev_uuas:
            lr /= 10
            optimizer = Adam(params, lr=lr)
            last_dev_uuas = dev_uuas
